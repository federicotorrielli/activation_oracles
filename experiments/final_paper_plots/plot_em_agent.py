...
VARIANTS: List[Tuple[str, str]] = [
    ("agent_mi0", "ADL"),
    ("talkative_mi0", "Activation Oracle"),
    ("baseline_mi0", "Blackbox"),
]
...
def visualize_grades_by_type_average(
    ...
    *,
    ...
    colors_for_variant: Dict[str, str] = {
        "agent_mi0": "#FDB813",  # Best White Box Method color
        "talkative_mi0": "#FDB813",  # Talkative Probe color (same as white box, but with hatch)
        "baseline_mi0": "#1f77b4",  # Best Black Box Method color
    }
    ...
    # Add method names below each bar
    for i, v_key in enumerate(variant_keys):
        for idx, t in enumerate(types):
            x_center = centers[idx] + offsets[i]
            label_text = variant_labels[i]
            ax.text(
                x_center,
                0.8,
                label_text,
                ha="center",
                va="top",
                fontsize=FONT_SIZE_X_AXIS_LABEL,
                rotation=0,
            )

    ax.set_xticks([])  # Remove X tick markers
    ax.set_xticklabels([])  # Remove "EM" subtitle
