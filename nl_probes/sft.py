...
def _patch_gemma4():
    ...
    2. Patch create_causal_mask_mapping to default mm_token_type_ids to zeros
       (required by 26B-A4B and 31B models which use bidirectional vision attention).
    """
    try:
        import torch
...
def push_lora_to_hf(
    ...
    try:
        ...
        model_short = original_model_name.split("/")[-1]
...
# Activation Oracle: {model_short}

This is a **LoRA adapter** that turns [{model_short}](https://huggingface.co/{original_model_name})
...
def oom_preflight_check(
    ...
    for _ in tqdm(range(3), desc="OOM preflight check"):
        loss = train_features_batch(
            cfg,
            largest_possible_batch,
            model,
            submodule,
            device,
            dtype,
...
def train_model(
    ...
    if cfg.use_lora and cfg.load_lora_path is None:
        ...
        model = PeftModel.from_pretrained(
            model, load_lora_path, is_trainable=True, autocast_adapter_dtype=True
        )
    ...
    if effective_steps != len(training_data):
        print(
            f"Trimming training_data from {len(training_data)} to {effective_steps} for equal DDP steps"
        )
    ...
    assert steps_per_epoch > 0, (
        "No optimizer steps will be run; check dataset/batch/accumulation sizes"
    )
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for step_idx, start in enumerate(
            ...
            loss = train_features_batch(
                ...
                dtype,
                do_backward=True,
                loss_scale=cfg.gradient_accumulation_steps,
            )
            ...
            if is_update_step:
                ...
                # -------------------------------- evaluation --------------------------------
                if global_step % cfg.eval_steps == 0 and (
                    cfg.eval_on_start or global_step > 0
                ):
                    if rank == 0:
                        eval_all_datasets(
                            cfg,
...
def build_datasets(
    ...
    if p is not None:
        if p >= 1.0 or p <= 0.0:
            raise ValueError(
                "max_len_percentile must be less than 1.0 and greater than 0.0"
            )

        lengths = sorted(len(td.input_ids) for td in all_training_data)
        median_length = lengths[len(lengths) // 2]
        print(
            f"Max length: {lengths[-1]}, Min length: {lengths[0]}, Median length: {median_length}"
        )
        ...
        all_training_data = [
            td for td in all_training_data if len(td.input_ids) <= threshold
        ]
        removed = before - len(all_training_data)
        print(
            f"Percentile trim: kept <= {threshold} tokens (p={p:.6f}). Removed {removed}/{before} examples."
        )
    ...
    if window_mult is not None:
        all_training_data = length_grouped_reorder(
            all_training_data, cfg.train_batch_size, window_mult
        )
...
def build_loader_groups(
    *,
    ...
    for layer_percent in layer_percents:
        sft_data_path = f"sae_data/qwen_hard_negatives_0_20000_layer_percent_{layer_percent}_sft_data_gpt-5-mini-2025-08-07.jsonl"
...
if __name__ == "__main__":
    ...
    models = [
        ...
        # "Qwen/Qwen3-4B",
        "google/gemma-4-E2B-it",  # ~10 GB BF16
        "google/gemma-4-E4B-it",  # ~16 GB BF16
        "google/gemma-4-26B-A4B-it",  # MoE 25.2B
        "google/gemma-4-31B-it",  # dense 30.7B
    ]

    from huggingface_hub import repo_exists, whoami as hf_whoami

    # Resolve HF owner once for skip checks
    try:
        _hf_user = hf_whoami()
        _hf_owner = _hf_user.get("name") if isinstance(_hf_user, dict) else None
    except Exception:
        _hf_owner = None

    for model_name in models:
        hf_repo_name = f"activation-oracle-{model_name.split('/')[-1]}"
        hf_full_repo = f"{_hf_owner}/{hf_repo_name}" if _hf_owner else hf_repo_name

        # Skip if already trained and pushed to HF
        if local_rank == 0 and repo_exists(hf_full_repo):
            print(f"Skipping {model_name} — {hf_full_repo} already exists on HF")
            continue
        ...
        print(
            f"Per-rank train batch size: {train_batch_size}, world size: {world_size}"
        )
        ...
        for hyperparam_override in iterations:
            ...
            if hyperparam_override["load_lora_path"] is not None:
                assert os.path.exists(hyperparam_override["load_lora_path"]), (
                    f"{hyperparam_override['load_lora_path']}"
                )
            ...
            print(
                f"training data length: {len(all_training_data)}, eval data length: {len(all_eval_data)}"
            )
